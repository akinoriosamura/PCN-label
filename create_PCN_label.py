    ...
    def convert_data(self):
        ...
        if debug:
            ...
            for x, y in (landmark + 0.5).astype(np.int32):
                cv2.circle(img_tmp, (x, y), 2, (255, 0, 0))
            cv2.imwrite("./sample_resized0.jpg", img_tmp)
        # PCNで回転角を取得し正座画像に
        # new_box_dict: {'x': 273, 'y': 231, 'w': 321, 'h': 321}
        # angle: (-180, 180)
        time.sleep(1)
        face_count, windows = get_PCN_result(imgT, detector)
        label_dicts, angles = get_label_dict(face_count, windows)
        # landmark[0]と一番近い左上bbをもつlabelを採用
        land_chin0x = landmark[0][0]
        label_dict = None
        angle = None
        pre_chin0 = 10000
        # import pdb;pdb.set_trace()
        ...
        if angle != None:
            ...
            imgT_height, imgT_width, _ = imgT.shape
            cro_dx = max(0, -croped_x1)
            cro_dy = max(0, -croped_y1)
            cro_edx = max(0, croped_x2 - imgT_width)
            cro_edy = max(0, croped_y2 - imgT_height)
            if debug:
                # 表示して確認
                img_tmp = imgT.copy()
                cv2.rectangle(img_tmp, (croped_x1, croped_y1), (croped_x2, croped_y2), (255, 0, 0), 1, 1)
                for x, y in (landmark + 0.5).astype(np.int32):
                    cv2.circle(img_tmp, (x, y), 2, (255, 0, 0))
                cv2.circle(img_tmp, (pcn_center_x, pcn_center_y), 3, (0, 255, 0))
                cv2.imwrite("./sample_resized_prenewimg.jpg", img_tmp)
            self.new_img = imgT[croped_y1:croped_y2, croped_x1:croped_x2]
            if (cro_dx > 0 or cro_dy > 0 or cro_edx > 0 or cro_edy > 0):
                # 足りない分は画像をコピーし周りに境界を作成
                self.new_img = cv2.copyMakeBorder(self.new_img, cro_dy, cro_edy, cro_dx, cro_edx, cv2.BORDER_REPLICATE)
            ...
            if debug:
                ...
                try:
                    ...
                    cv2.rectangle(img_tmp, (self.new_box[0], self.new_box[1]), (self.new_box[0] + self.new_box[2], self.new_box[1] + self.new_box[3]), (255, 0, 0), 1, 1)
                    for x, y in (self.new_landmark + 0.5).astype(np.int32):
                        cv2.circle(img_tmp, (x, y), 2, (255, 0, 0))
                    cv2.circle(img_tmp, (pcn_center_x-croped_x1, pcn_center_y-croped_y1), 3, (0, 255, 0))
                    cv2.imwrite("./sample_resized_postnewland.jpg", img_tmp)
                except:
                    import pdb;pdb.set_trace()
                    ...
                    try:
                        ...
                        for land in self.new_landmark:
                            for each_land in land:
                                self.label.append(str(int(each_land)))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        ...
        # ┗ txt: 175 label = [136(68*2) points] + [4 bbox] + [6 attributes] + [28(14*2) pcn landmark] saveName
        pcn_type = sys.argv[1]
    else:
